...
// ============================================================
// IMPORTANT: Use port 5035 first (more likely to work on Render)
// ============================================================
const WS_ENDPOINTS = [
    'wss://demo.ctraderapi.com:5035',  // Try port 5035 first (often less blocked)
    'wss://demo.ctraderapi.com:5036',  // Fallback to 5036
    'wss://demo.ctraderapi.com:5035/websocket',
    'wss://demo.ctraderapi.com:5036/websocket'
];
...
// ============================================================
// CTRADER WEBSOCKET CONNECTION - Port 5035 FIRST
// ============================================================
...
function connectToCtrader() {
    ...
    if (reconnectAttempts >= MAX_RECONNECT_ATTEMPTS) {
        ...
        console.log('💡 Please restart the service or get a new token');
        return;
    }
    
    // Try next endpoint if we've exhausted all
    if (currentEndpointIndex >= WS_ENDPOINTS.length) {
        console.log('❌ All WebSocket endpoints failed');
        reconnectAttempts++;
        currentEndpointIndex = 0;
        const waitTime = Math.min(30000, 5000 * reconnectAttempts);
        console.log(`🔄 Retry ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} in ${waitTime/1000}s...`);
        setTimeout(connectToCtrader, waitTime);
        return;
    }
    
    const endpoint = WS_ENDPOINTS[currentEndpointIndex];
    const wsUrl = `${endpoint}?access_token=${userTokens.accessToken}`;
    
    console.log(`🔌 Connecting to cTrader (${currentEndpointIndex + 1}/${WS_ENDPOINTS.length}): ${endpoint}`);
    console.log('🔑 Token:', userTokens.accessToken.substring(0, 20) + '...');
    ...
    try {
        ...
        const timeout = setTimeout(() => {
            if (ctraderWs && ctraderWs.readyState !== WebSocket.OPEN) {
                ...
                setTimeout(connectToCtrader, 1000);
            }
        }, 15000);
        ...
        ctraderWs.on('error', (error) => {
            clearTimeout(timeout);
            console.log(`❌ WebSocket error on ${endpoint}:`, error.message || 'Unknown');
            // Don't immediately try next - let close handle it
        });
        
        ctraderWs.on('close', (event) => {
            clearTimeout(timeout);
            stopHeartbeat();
            ...
            // If not connected and we have more endpoints, try next
            if (!isConnected && currentEndpointIndex < WS_ENDPOINTS.length - 1) {
                currentEndpointIndex++;
                console.log(`🔄 Trying next endpoint (${currentEndpointIndex + 1}/${WS_ENDPOINTS.length})`);
                setTimeout(connectToCtrader, 2000);
            } else if (!isConnected) {
                // All endpoints tried, increment retry count
                reconnectAttempts++;
                currentEndpointIndex = 0;
                const waitTime = Math.min(30000, 5000 * reconnectAttempts);
                console.log(`🔄 Reconnect attempt ${reconnectAttempts}/${MAX_RECONNECT_ATTEMPTS} in ${waitTime/1000}s...`);
                setTimeout(connectToCtrader, waitTime);
            }
        });
...
function startHeartbeat() {
    ...
    heartbeatInterval = setInterval(() => {
        if (ctraderWs && ctraderWs.readyState === WebSocket.OPEN) {
            ...
            try {
                const pingMsg = JSON.stringify({ type: 'ProtoOAPing', requestId: ++requestId });
                ctraderWs.send(pingMsg);
...
function authenticateApplication() {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.log('❌ Cannot authenticate: WebSocket not open');
        return;
    }
...
function getAccountList() {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.log('❌ Cannot get accounts: WebSocket not open');
        return;
    }
...
function authenticateAccount(accountId) {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.log('❌ Cannot authenticate account: WebSocket not open');
        return;
    }
...
function sendCtraderMessage(type, data) {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.error(`❌ Cannot send ${type}: WebSocket not open`);
        return null;
    }
