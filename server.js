...
app.get('/oauth/callback', async (req, res) => {
    ...
    try {
        ...
        // Reset endpoint index and try connecting
...
// ============================================================
// CTRADER WEBSOCKET CONNECTION - WITH FALLBACK ENDPOINTS
// ============================================================
...
function connectToCtrader() {
    ...
    if (currentEndpointIndex >= WS_ENDPOINTS.length) {
        ...
        setTimeout(connectToCtrader, 10000);
        return;
    }
    ...
    console.log(`🔌 Connecting to cTrader WebSocket (${currentEndpointIndex + 1}/${WS_ENDPOINTS.length}): ${endpoint}`);
    ...
    try {
        ...
        ctraderWs.on('open', () => {
            ...
            isConnected = true;
            currentEndpointIndex = 0; // Reset on success
        ...
        ctraderWs.on('close', (event) => {
            ...
            if (!isConnected && currentEndpointIndex < WS_ENDPOINTS.length - 1) {
                ...
                // All endpoints tried, wait and retry from start
                currentEndpointIndex = 0;
                console.log('🔄 All endpoints tried, restarting in 30s...');
                setTimeout(connectToCtrader, 30000);
            }
        });
        ...
        switch (parsed.type) {
            ...
            case 'ProtoOAGetAccountListByAccessTokenRes':
                if (parsed.error) {
                    ...
                    // Try to get account from token info
