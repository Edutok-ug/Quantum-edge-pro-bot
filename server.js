...
app.get('/oauth/callback', async (req, res) => {
    ...
    try {
        ...
        console.log('🔌 Auto-connecting to cTrader...');
        setTimeout(() => {
            connectToCtrader();
        }, 2000);
            ...
            <body>
                <div class="box">
                    ...
                    <div class="info" style="margin-top:15px;color:#f59e0b;">
                        🔌 Auto-connecting to cTrader WebSocket... Check the logs.
                    </div>
...
// ============================================================
// CTRADER WEBSOCKET CONNECTION - WITH BETTER DEBUGGING
// ============================================================

function connectToCtrader() {
    if (ctraderWs && (ctraderWs.readyState === WebSocket.OPEN || ctraderWs.readyState === WebSocket.CONNECTING)) {
    ...
    connectionAttempts++;
    console.log(`🔌 Connecting to cTrader WebSocket (attempt ${connectionAttempts}/${MAX_RECONNECT_ATTEMPTS})...`);
    console.log(`📡 WebSocket URL: ${WS_DEMO_URL}`);
    console.log(`🔑 Token: ${userTokens.accessToken.substring(0, 20)}...`);
    
    try {
        // Add token to WebSocket connection
        const wsUrl = `${WS_DEMO_URL}?access_token=${userTokens.accessToken}`;
        ...
        ctraderWs.on('error', (error) => {
            ...
            isConnected = false;
            broadcastToClients({ type: 'error', message: error.message || 'WebSocket error' });
        });
        ...
        ctraderWs.on('close', (event) => {
            ...
            if (connectionAttempts < MAX_RECONNECT_ATTEMPTS) {
                ...
                setTimeout(connectToCtrader, delay);
            }
        });
...
function getAccountList() {
    ...
    if (!userTokens.accessToken) {
        console.log('❌ Cannot get accounts: No access token');
        return;
    }
...
function authenticateAccount(accountId) {
    ...
    if (!userTokens.accessToken) {
        console.log('❌ Cannot authenticate account: No access token');
        return;
    }
...
function getPositions() {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) return;
    if (!selectedAccount) {
        console.log('❌ Cannot get positions: No account selected');
        return;
    }
...
function placeOrder(orderData) {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.log('❌ Cannot place order: WebSocket not open');
...
function closePosition(positionId) {
    if (!ctraderWs || ctraderWs.readyState !== WebSocket.OPEN) {
        console.log('❌ Cannot close position: WebSocket not open');
        return { error: 'Not connected to cTrader' };
    }
...
function handleCtraderMessage(data) {
    try {
        ...
        console.log('📨 Received:', parsed.type || 'unknown', parsed.requestId || '');
