...
app.get('/oauth/callback', async (req, res) => {
    ...
    try {
        ...
        // Parse token response - cTrader uses these keys
        ...
        if (!accessToken) {
            throw new Error('No access token received: ' + JSON.stringify(responseData));
        }
        ...
        console.log('🔌 Auto-connecting to cTrader WebSocket...');
        setTimeout(() => {
            connectToCtrader();
        }, 2000);
        ...
        res.send(`
            ...
            <body style="background:#0a0e1a;color:#e2e8f0;font-family:sans-serif;padding:40px;">
                ...
                <pre style="background:#0a1222;padding:15px;border-radius:8px;overflow:auto;max-height:300px;">${error.message}\n${JSON.stringify(error.response?.data || '', null, 2)}</pre>
                <a href="/auth/ctrader" style="color:#f59e0b;">🔐 Try Again</a>
                <br><a href="/" style="color:#f59e0b;">← Back</a>
            </body>
            </html>
...
function connectToCtrader() {
    ...
    // IMPORTANT: cTrader requires token as query parameter
    const wsUrl = `${WS_DEMO_URL}?access_token=${userTokens.accessToken}`;
    console.log('📡 WebSocket URL:', wsUrl.replace(userTokens.accessToken, 'HIDDEN_TOKEN'));
